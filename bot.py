...
async def message_button(update, context):
    ...
    prompt = load_prompt(query)
    user_chat_history = "\n\n".join(dialog.list)
    my_message = await send_text(update, context, "Чат ГПТ думает над вариантами ответов...")
...
async def opener_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        user_info = dialog_user_info_to_str(dialog.user)
        my_message = await send_text(update, context, "ChatGPT генерирует Opener...")
        answer = await chatgpt.send_question(prompt, user_info)
        await my_message.edit_text(answer)
...
async def hello_button(update, context):
    ...
    await update.callback_query.answer()
    # await send_text(update, context, f"Вы нажали на кнопку {query}")
